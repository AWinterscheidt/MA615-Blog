...
All these steps can be seen in our [load_and_clean_data.R](/load_and_clean_data.R) file. Additional packages used were `naniar` and `haven`. The former was used to replace non-response codes in the ANES data set with NAs, and the former latter was used to import .dta files, as earlier ANES data sets had such file types. 
